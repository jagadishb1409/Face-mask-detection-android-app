...
def export(weights='./yolov5s.pt',  # weights path
    ...
    if 'torchscript' in include or 'coreml' in include:
        ...
        try:
            ...
            ts_optim = optimize_for_mobile(ts)
            ts_optim._save_for_lite_interpreter(f)
            print(f'wget --no-check-certificate https://shapenet.cs.stanford.edu/media/minhyuk/spfn/data/spfn_traceparts.zip{prefix} export success, saved as {f} ({file_size(f):.1f} MB)')
